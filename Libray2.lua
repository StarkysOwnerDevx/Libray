...
function Kavo.CreateLib(kavName, themeList)
    ...
    function Tabs:NewTab(tabName)
        ...
        tabButton.MouseButton1Click:Connect(function()
            ...
            if isMobile then
                Utility:TweenObject(tabButton, {BackgroundColor3 = Color3.fromRGB(themeList.SchemeColor.r * 255 + 20, themeList.SchemeColor.g * 255 + 20, themeList.SchemeColor.b * 255 + 20)}, 0.1)
            end
        end)
        ...
        function Sections:NewSection(secName, hidden)
            ...
            function Elements:NewSlider(slidInf, slidTip, maxvalue, minvalue, callback)
                ...
                if isMobile then
                    ...
                    sliderBtn.TouchStarted:Connect(function(touch)
                        touchStartPos = touch.Position
                        touchStartTime = tick()
                    end)
                    
                    sliderBtn.TouchEnded:Connect(function(touch)
                        if touchStartPos then
                            local touchEndPos = touch.Position
                            local deltaX = touchEndPos.X - touchStartPos.X
                            local newSize = math.clamp(sliderDrag.AbsoluteSize.X + deltaX, 0, 149)
                            Value = math.floor((((tonumber(maxvalue) - tonumber(minvalue)) / 149) * newSize) + tonumber(minvalue))
                            pcall(function()
                                callback(Value)
                            end)
                            sliderDrag:TweenSize(UDim2.new(0, newSize, 0, 6), "InOut", "Linear", 0.1, true)
                        end
                    end)
                end
            ...
            function Elements:NewDropdown(dropname, dropinf, list, callback)
                ...
                if isMobile then
                    dropOpen.TouchTap:Connect(function()
                        if not focusing then
                            if opened then
                                opened = false
                                dropFrame:TweenSize(UDim2.new(0, 352, 0, 33), "InOut", "Linear", 0.08)
                ...
                for i,v in next, list do
                    ...
                    local function optionAnimateClick()
